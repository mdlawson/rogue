...
window.require.define({"state": function(exports, require, module) {
  (function() {
    ...
    state = {
      setup: function(game) {
        var assets, bg1, bg2, blocks, tiles, x, _ref;
      ...
      update: function(game, dt) {
        ...
        player = game.player;
        input = game.input;
        if (input.pressed("right")) player.move(2, 0);
        if (input.pressed("left")) player.move(-2, 0);
        if (input.pressed("up")) {
          if (player.canJump) {
        ...
        if (input.pressed("down")) player.move(0, 2);
