...
(function() {
  var Animation, AssetManager, Entity, Game, GameLoop, KeyboardManager, Mouse, Rogue, RollingAverage, SoundBox, SpriteSheet, TileMap, Tween, ViewPort, c, collision, find, gfx, log, math, util,
  ...
  c.tween = (function() {
    ...
    tween.prototype.init = function() {
      this.tweening = false;
      this.tweens = [];
      return this.updates[95] = function() {
    ...
    tween.prototype.tween = function(props, time) {
      this.tweens.push(new Tween(this, props, time));
      return this;
    };
  ...
  Tween = (function() {
    ...
    function Tween(target, result, time) {
      var prop, val, _ref;
      this.target = target;
      this.result = result;
      this.iter = {};
      ...
      for (prop in _ref) {
        val = _ref[prop];
        if (!(isNaN(val) || isNaN(this.target[prop]))) {
          this.iter[prop] = (val - this.target[prop]) / this.t;
        } else {
    ...
    Tween.prototype.run = function() {
      ...
      for (prop in _ref) {
        val = _ref[prop];
        if (prop === "x") {
          this.target.move(val, 0);
        } else if (prop === "y") {
          this.target.move(0, val);
        }
        _results.push(this.target[prop] += val);
      }
      return _results;
    };

    Tween.prototype.tl = function() {
      return this.t--;
    };
